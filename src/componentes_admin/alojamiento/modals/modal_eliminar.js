...
export default function ModalEliminar(props) {
    ...
    async function eliminarAlojamiento() {
        ...
        const respuesta = await gestorAxios.delete('/alojamiento/eliminar/' + data, {
            headers: {
        ...
        if (respuesta.status === 200) {
            ...
            setMsgSnack({
                contenido: 'Se elimino correctamente el Alojamiento',
                severity: 'success'
            });
            setOpenSnack(true);
        } else {
    ...
    return (<>
        ...
        <DialogContent>
            <DialogContentText>
                ¿Esta seguro que desea eliminar esta Alojamiento permanentemente?
            </DialogContentText>
        </DialogContent>
