...
export default function ModalEliminar(props) {
    ...
    async function eliminarTipoUsuario() {
        ...
        const respuesta = await gestorAxios.delete('/tipo-usuario/eliminar/' + data, {
            headers: {
        ...
        if (respuesta.status === 200) {
            ...
            setMsgSnack({
                contenido: 'Se elimino correctamente el Rango de Usuario',
                severity: 'success'
            });
            setOpenSnack(true);
        } else {
    ...
    return (<>
        ...
        <DialogContent>
            <DialogContentText>
                ¿Esta seguro que desea eliminar esta Rango permanentemente?
            </DialogContentText>
        </DialogContent>
