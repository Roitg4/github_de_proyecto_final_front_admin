...
export default function ModalEliminar(props) {
    ...
    async function eliminarAlojamientoTipo() {
        ...
        const respuesta = await gestorAxios.delete('/alojamiento-estado/eliminar/' + data, {
            headers: {
        ...
        if (respuesta.status === 200) {
            ...
            setMsgSnack({
                contenido: 'Se elimino correctamente el Estado de Alojamiento',
                severity: 'success'
            });
            setOpenSnack(true);
        } else {
    ...
    return (<>
        ...
        <DialogTitle sx={{ fontSize: 'h5.fontSize' }}>
            Eliminar Estado del Alojamiento
            <IconButton
                aria-label="close"
                onClick={onClose}
                sx={{
        ...
        <DialogContent>
            <DialogContentText>
                ¿Esta seguro que desea eliminar esta Estado permanentemente?
            </DialogContentText>
        </DialogContent>
