...
export default function ModalEliminar(props) {
    ...
    async function eliminarAlojamientoTipo() {
        ...
        const respuesta = await gestorAxios.delete('/alojamiento-tipo/eliminar/' + data, {
            headers: {
        ...
        if (respuesta.status === 200) {
            ...
            setMsgSnack({
                contenido: 'Se elimino correctamente el Tipo de Alojamiento',
                severity: 'success'
            });
            setOpenSnack(true);
        } else {
    ...
    return (<>
        ...
        <DialogContent>
            <DialogContentText>
                ¿Esta seguro que desea eliminar esta Tipo permanentemente?
            </DialogContentText>
        </DialogContent>
